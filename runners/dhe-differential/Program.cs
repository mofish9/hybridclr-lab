...
if (args.Length != 4 || args[0] != "reference")
    throw new ArgumentException("Use instrument <prepared root> <new root>, or reference <case DLL> <support root> <new result binary>.");
...
{
    format = "hybridclr.dhe-differential-reference.json", schemaVersion = 1,
...
Console.WriteLine("CLR differential observations recorded: " + result.Cases.Length);
return 0;
