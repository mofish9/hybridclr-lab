function Get-TreeHash([string]$Root, [string[]]$IgnoreRelativePaths = @()) {
    ...
    foreach ($ignoredPath in @($IgnoreRelativePaths)) {
        if (-not [string]::IsNullOrWhiteSpace([string]$ignoredPath)) {
            $null = $ignored.Add(([string]$ignoredPath).Replace('\', '/').TrimStart('/'))
        }
    }
    $filesByRelativePath = @{}
...
function Get-TreeHashExcludingGit([string]$Root, [string[]]$IgnoreRelativePaths = @()) {
    $resolvedRoot = [IO.Path]::GetFullPath($Root).TrimEnd('\', '/')
    if (-not (Test-Path -LiteralPath $resolvedRoot -PathType Container)) {
        throw "Tree hash root was not found: $resolvedRoot"
    }
...
# Unity may regenerate metadata for editor-only files that are not supported
# by the selected editor version. Those generated .meta contents are not part
# of the package source identity; all executable/package content remains
# covered by the tree hash.
$script:DhePackageHashIgnoredPaths = @(
    'Editor/BuildProcessors/AddLil2cppSourceCodeToXcodeproj2023OrNewer.cs.meta'
)

function Get-DhePackageTreeHash([string]$Root) {
    return Get-TreeHash -Root $Root -IgnoreRelativePaths $script:DhePackageHashIgnoredPaths
}

function Get-DhePackageHashIgnoredPaths {
    return @($script:DhePackageHashIgnoredPaths)
}

function Test-DhePackageHashPolicy($Lock) {
    if ($null -eq $Lock) { return $false }
    $property = $Lock.PSObject.Properties["treeHashIgnoredPaths"]
    if ($null -eq $property) { return $false }
    $actual = @($property.Value | ForEach-Object {
        if ($null -ne $_) { ([string]$_).Replace('\\', '/').TrimStart('/') }
    } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    $expected = @(Get-DhePackageHashIgnoredPaths)
    return ($actual -join "`n") -eq ($expected -join "`n")
}

function Get-DhePackageSourceTreeHash([string]$Root) {
    return Get-TreeHashExcludingGit -Root $Root -IgnoreRelativePaths $script:DhePackageHashIgnoredPaths
}
