...
namespace HybridCLR.Lab.Editor
{
    public static class CurrentStorageProbeBuild
    {
        ...
        {
            string output = Argument("-probeOutput");
            string player = Path.Combine(output, "player", "CurrentStorage.exe");
            ...
            File.WriteAllText(Path.Combine(output, "probe-build.json"), JsonUtility.ToJson(new BuildReceipt
            { passed = true, unityVersion = Application.unityVersion, bytes = report.summary.totalSize.ToString() }, true));
        }
