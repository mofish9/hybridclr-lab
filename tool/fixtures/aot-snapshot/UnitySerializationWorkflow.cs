...
internal static class UnitySerializationWorkflow
{
    ...
    {
        if (args.Length != 6 || args[5] != "read" && args[5] != "full")
            throw new ArgumentException("unity-serialization-replay <lab> <tool.dll> <Base proof> <resource workflow output> <new output> <read|full>");
        ...
        Execute(player, true, "-batchmode", "-nographics", "-snapshotResult", reportPath, "-snapshotResourceRoot", stage,
            "-expectedRevision", "73", "-expectedAssemblies", "4", "-expectedModuleConstant", "202", "-unitySerializationProbe", "true", "-logFile", log);
        ...
        string[] checks = lines.Where(line => line.StartsWith("DHE Unity serialization check: ")).Select(line => line.Substring("DHE Unity serialization check: ".Length)).ToArray();
        ...
        bool passed = immutable && report.GetProperty("passed").GetBoolean() && checks.SequenceEqual(sequence) &&
            lines.Count(line => line == "DHE Unity serialization pass: " + sequence.Length) == 1 &&
        ...
        File.WriteAllText(Path.Combine(output, "result.json"), JsonSerializer.Serialize(new { passed, immutable, checks, expected = sequence,
            ...
            scope = args[5] == "read" ? "Native JSON read diagnostic only; complete serialization is still required" : "Native JSON/overwrite/clone assertions on an immutable Base"
