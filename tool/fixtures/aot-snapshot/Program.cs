...
if (args.Length > 0 && args[0] == "parent-evolution-reference") return ParentEvolutionWorkflow.Reference(args.Skip(1).ToArray());
if (args.Length > 0 && args[0] == "parent-evolution-policy") return ParentEvolutionPolicyTests.Run(args.Skip(1).ToArray());
if (args.Length > 0 && args[0] == "parent-evolution-replay") return ParentEvolutionWorkflow.Replay(args.Skip(1).ToArray());
if (args.Length > 0 && args[0] == "parent-transition-reference") return ParentTransitionWorkflow.Reference(args.Skip(1).ToArray());
if (args.Length > 0 && args[0] == "parent-transition-replay") return ParentTransitionWorkflow.Replay(args.Skip(1).ToArray());
...
if (args.Length > 0 && args[0] == "replay-frozen-entry") return FrozenEntryWorkflow.Replay(args.Skip(1).ToArray());
if (args.Length > 0 && args[0] == "recompile-frozen-entry") return FrozenEntryWorkflow.Recompile(args.Skip(1).ToArray());
